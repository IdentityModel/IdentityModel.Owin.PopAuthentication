properties {
	...
	$src_directory = "$base_directory\src\IdentityModel.Owin.PopAuthentication"
	$output_directory = "$base_directory\build"
	$dist_directory = "$base_directory\distribution"
	$sln_file = "$base_directory\IdentityModel.Owin.PopAuthentication.sln"
...
task RunTests -depends Compile {
	$project = "IdentityModel.Owin.PopAuthentication.Tests"
...
task CreateNuGetPackage -depends Compile {
	...
	copy-item $base_directory\IdentityModel.Owin.PopAuthentication.nuspec $dist_directory

	new-item $dist_directory\lib\net45\ -type directory
	copy-item $output_directory\IdentityModel.Owin.PopAuthentication.dll $dist_directory\lib\net45\
	copy-item $output_directory\IdentityModel.Owin.PopAuthentication.pdb $dist_directory\lib\net45\

	exec { . $nuget_path pack $dist_directory\IdentityModel.Owin.PopAuthentication.nuspec -BasePath $dist_directory -o $dist_directory -version $packageVersion }
}
